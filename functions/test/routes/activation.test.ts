...
describe("GET /v1/device-activation", () => {
  ...
  it("resource existence: pairing code not found returns 404", async () => {
    ...
    expect(res.json()).toEqual({ error: "Pairing code not found" });
    await app.close();
  });
  ...
  it("error mapping: service error preserves status + body", async () => {
    ...
    expect(res.json()).toEqual({ error: "unavailable", message: "upstream down" });
    await app.close();
  });
...
describe("POST /v1/device-activation/authorize", () => {
  ...
  it("auth: second factor required returns 401", async () => {
    ...
    expect(res.statusCode).toBe(401);
    expect(res.json()).toEqual({ error: "second_factor_required" });
    await app.close();
  });
  ...
  it("error mapping: service error preserves status + body", async () => {
    ...
    expect(res.json()).toEqual({ error: "gone", message: "Pairing code expired" });
    await app.close();
  });
});
